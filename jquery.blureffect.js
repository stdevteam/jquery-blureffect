...
(function($) {
    jQuery.fn.blureffect = function(options) {
        // merge default and user parameters
        options = $.extend($.fn.blureffect.defaults, options);
     ...
     * @type Object
     */
    jQuery.fn.blureffect.processor = {
        mul_table: [
        ...
        blurImg: function(imageId, canvasId, radius, regions, alpha){
            if(regions.length <= 0){
                //no regions
                return;
            }

            var img = document.getElementById(imageId), w = img.naturalWidth, 
            h = img.naturalHeight, canvas = document.getElementById( canvasId );
            ...
            for(var i in regions){
                region = regions[i];
                var startX = region.startX, startY = region.startY, 
                    blurWidth = region.width, blurHeight = region.height;

                if(alpha){
                    this.blurCanvasRGBA(canvasId, startX, startY, blurWidth, blurHeight, radius);
                }else{
                    this.blurCanvasRGB(canvasId, startX, startY, blurWidth, blurHeight, radius);
                }
            }
        },

        blurCanvasRGBA: function(id, top_x, top_y, width, height, radius){
            if(isNaN(radius) || radius < 1 ){
                return;
            }
            radius |= 0;
        ...
        blurCanvasRGB: function(id, top_x, top_y, width, height, radius){
            if(isNaN(radius) || radius < 1 ){
                return;
            }
            radius |= 0;
